...
configsvr_name=$(echo "$describe_result" | jq -r '[.replsets[]? | select((.configsvr // false) == true or (.iscs // false) == true) | .name] | .[0] // empty')
shardsvr_names=$(echo "$describe_result" | jq -r '[.replsets[]? | select(((.configsvr // false) != true) and ((.iscs // false) != true)) | .name] | join(",")')
if [ -n "$configsvr_name" ]; then
  extras=$(buildJsonString $extras "configsvr" "$configsvr_name")
fi
if [ -n "$shardsvr_names" ]; then
  extras=$(buildJsonString $extras "shardsvr" "$shardsvr_names")
fi
