...
function upload_continuous_backup_info() {
  ...
  local pitr_nodes
  pitr_nodes=$(echo "$status_result" | jq -r '.nodes?[]?')
  local shardsvr=""
  local configsvr=""
  ...
  if [ -z "$shardsvr" ] || [ -z "$configsvr" ]; then
    echo "INFO: PITR node list is incomplete, falling back to pbm status cluster metadata."
    local pbm_status_result
    pbm_status_result=$(pbm status --mongodb-uri "$PBM_MONGODB_URI" -o json)
    if [ -z "$shardsvr" ]; then
      shardsvr=$(echo "$pbm_status_result" | jq -r '[.cluster[]? | .rs | select(contains("config-server") | not)] | join(",")')
    fi
    if [ -z "$configsvr" ]; then
      configsvr=$(echo "$pbm_status_result" | jq -r '[.cluster[]? | .rs | select(contains("config-server"))] | join(",")')
    fi
  fi
