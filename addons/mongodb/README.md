...
spec:
  ...
  componentSpecs:
    - name: mongodb
      ...
      # Valid options are: [8.0.8,8.0.6,8.0.4,7.0.19,7.0.16,7.0.12,6.0.22,6.0.20,6.0.16,5.0.30,5.0.28,4.4.29,4.2.24,4.0.28]
...
```yaml
# cat examples/mongodb/cluster-hostnetwork.yaml
apiVersion: apps.kubeblocks.io/v1
kind: Cluster
metadata:
  name: mongo-hostnetwork
  namespace: demo
  annotations:
    # `kubeblocks.io/host-network` is a reserved annotation
    # it defines the feature gate to enable the host-network for specified components or shardings.
...
```yaml
# cat examples/mongodb/scale-in-specified-pod.yaml
apiVersion: operations.kubeblocks.io/v1alpha1
kind: OpsRequest
metadata:
  name: mongo-scale-in-specified-pod
  namespace: demo
spec:
  # Specifies the name of the Cluster resource that this operation is targeting.
  clusterName: mongo-cluster
...
```yaml
# cat examples/mongodb/backup-dump.yaml
apiVersion: dataprotection.kubeblocks.io/v1alpha1
kind: Backup
metadata:
  name: mongo-cluster-backup-dump
  namespace: demo
spec:
  # Specifies the backup method name that is defined in the backup policy.
  # - dump
...
```yaml
# cat examples/mongodb/restore-pitr.yaml
apiVersion: apps.kubeblocks.io/v1
kind: Cluster
metadata:
