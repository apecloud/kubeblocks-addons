...
# Normal startup: syncer's ReplicaDPEngine owns the physical-restore lifecycle.
exec mongod --bind_ip_all --port $PORT --replSet $CLUSTER_COMPONENT_NAME --config /etc/mongodb/mongodb.conf
