...
Describe "Valkey Cascade Self-Heal Daemon"
  ...
  Describe "cascade_check_one_round() — repair safeguards"
    setup() {
      ...
      valkey-cli() {
        # detect REPLICAOF subcommand
        for arg in "$@"; do
          case "$arg" in
            REPLICAOF)
              # next two args are host port
              shift 1
              # Note: shifting here won't affect outer scope; collect by index instead
              ;;
          esac
        done
        # Simpler: inspect raw command line
        local cmdline="$*"
        ...
        # Otherwise treat as INFO replication
        local call_index
    ...
    It "issues REPLICAOF when cascade detected and no guards trigger"
      ...
      When call cascade_check_one_round
      The status should be success
      The stderr should include "execute-replicaof"
