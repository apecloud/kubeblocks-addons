...
#
# Long-running cascade self-heal daemon, started from valkey-start.sh
# (sourced + `cascade_maintenance_loop &` before `exec valkey-server`).
# Lives in the valkey container.  After valkey-start.sh's exec, this daemon
# becomes a child of valkey-server (PID 1).  valkey-server does not reap
...
#
# What it does:
#   Every CASCADE_CHECK_INTERVAL_SECONDS, if this pod's local INFO replication
#   reports `role:slave`, query the configured master.  If that master is
#   itself a slave (cascading topology that Sentinel does not auto-correct),
#   issue REPLICAOF directly to the real master at the head of the chain.
#
# Stderr is captured by the kubelet from the valkey container's main process
# (after exec) and surfaced via `kubectl logs <pod> -c valkey`.  Decisions
# are observable without writing to /tmp.
...
# is_self_host — guard against issuing REPLICAOF to ourselves.
# In the valkey container the FQDN env is `POD_FQDN`; if a future cmpd change
# also sets `KB_POD_FQDN` we accept that too as a fallback.
...
# cascade_check_one_round — one detection-and-repair iteration.
#   1) Local INFO → only proceed if role=slave.
#   2) Remote INFO (timeout-bounded) → check master's role.
#   3) If master is slave AND we are still slave AND target is not self →
#      REPLICAOF real master.
cascade_check_one_round() {
  ...
  # Re-read local role to guard against stale-role race.
  ...
  if cascade_is_self_host "${real_master_host}"; then
    echo "WARNING: skip cascade repair (skip-self-target): target ${real_master_host}:${real_master_port:-${KB_SERVICE_PORT:-${SERVICE_PORT:-6379}}} resolves to current pod ${CURRENT_POD_NAME:-unknown}." >&2
    return 0
  fi

  local local_port="${KB_SERVICE_PORT:-${SERVICE_PORT:-6379}}"
  echo "WARNING: cascading topology (execute-replicaof) — our master ${master_host} is a slave of ${real_master_host}. Issuing REPLICAOF to reconnect directly to real master." >&2
