...
for pod in $(datasafed list /); do
  for file in $(datasafed list "/$pod/") ; do
    ...
    while true; do
      if datasafed pull "${file}" - | qdrant_curl --retry 3 -sS -f -X POST "${SCHEME}://${DP_DB_HOST}:6333/collections/${collection_name}/snapshots/upload?priority=snapshot" \
        -H 'Content-Type:multipart/form-data' \
        -F "snapshot=@-;filename=${snapshot}" > /tmp/qdrant-restore.log 2>&1; then
