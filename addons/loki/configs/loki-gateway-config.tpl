...
http {
  ...
  server {
    ...
    {{- $writeUrl    := printf "http://%s.%s.svc.%s:3100" $writeHost   .KB_NAMESPACE .clusterDomain }}
    {{- $readUrl     := printf "http://%s.%s.svc.%s:3100" $readHost    .KB_NAMESPACE .clusterDomain }}
    {{- $backendUrl  := printf "http://%s.%s.svc.%s:3100" $backendHost .KB_NAMESPACE .clusterDomain }}
