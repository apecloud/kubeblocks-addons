...
output=$(/pd-ctl -u "$PD_ADDRESS" $extraArg store delete addr "$TIKV_ADDRESS")
echo "$output"
...
# shellcheck disable=SC2086
until [[ $(/pd-ctl -u "$PD_ADDRESS" $extraArg store | jq "any(.stores[]; select(.store.address == \"$TIKV_ADDRESS\"))") == "false" ]]
do
...
# shellcheck disable=SC2086
/pd-ctl -u "$PD_ADDRESS" $extraArg store remove-tombstone
