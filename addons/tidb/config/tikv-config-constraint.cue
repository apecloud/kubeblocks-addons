#TIKVParameter: {
	...
	"log.file.max-size": int & <=4096 | *300
	...
	"server.status-thread-pool-size": int & >=1 | *1
	...
	"server.grpc-concurrency": int & >=1 | *5

	// The maximum number of concurrent requests allowed in a gRPC stream
	"server.grpc-concurrent-stream": int & >=1 | *1024
	...
	"server.grpc-raft-conn-num": int & >=1 | *1

	// Sets the maximum length of a gRPC message that can be sent. Unit: Bytes
	"server.max-grpc-send-msg-len": int & <=2147483647 | *10485760
	...
	"server.background-thread-count": int | *2
	...
	"server.raft-client-queue-size": int | *16384
	...
	"server.forward-max-connections-per-address": int | *4
	...
	"readpool.unified.max-tasks-per-worker": int & >=2 | *2000
	...
	"readpool.storage.normal-concurrency": int & >=1
	...
	"readpool.storage.low-concurrency": int & >=1
	...
	"readpool.storage.max-tasks-per-worker-high": int & >=2 | *2000
	...
	"readpool.storage.max-tasks-per-worker-normal": int & >=2 | *2000
	...
	"readpool.storage.max-tasks-per-worker-low": int & >=2 | *2000
	...
	"readpool.coprocessor.max-tasks-per-worker-high": int & >=2 | *2000
	...
	"readpool.coprocessor.max-tasks-per-worker-normal": int & >=2 | *2000
	...
	"readpool.coprocessor.max-tasks-per-worker-low": int & >=2 | *2000
	...
	"storage.scheduler-concurrency": int & >=1 | *524288
	...
	"storage.scheduler-worker-pool-size": int | *4
	...
	"storage.flow-control.memtables-threshold": int | *5
	...
	"storage.flow-control.l0-files-threshold": int | *20
	...
	"raftstore.raft-heartbeat-ticks": int | *2
	...
	"raftstore.raft-election-timeout-ticks": int | *10
	...
	"raftstore.raft-min-election-timeout-ticks": int & >=0 | *0
	...
	"raftstore.raft-max-election-timeout-ticks": int & >=0 | *0
	...
	"raftstore.raft-max-inflight-msgs": int & <=16384 | *256
	...
	"raftstore.raft-log-reserve-max-ticks": int | *6
	...
	"raftstore.region-compact-tombstones-percent": int & >=1 & <=100 | *30

	// The number of redundant MVCC rows required to trigger RocksDB compaction.
	"raftstore.region-compact-min-redundant-rows": int & >=0 | *50000

	// The percentage of redundant MVCC rows required to trigger RocksDB compaction.
	"raftstore.region-compact-redundant-rows-percent": int & >=1 & <=100 | *20
	...
	"raftstore.snap-generator-pool-size": int & >=1 | *2
	...
	"raftstore.notify-capacity": int & >=0 | *40960

	// The maximum number of messages processed per batch
	"raftstore.messages-per-tick": int & >=0 | *4096
	...
	"raftstore.leader-transfer-max-log-lag": int & >=10 | *128
	...
	"raftstore.merge-max-log-gap": int | *10
	...
	"raftstore.local-read-batch-size": int | *1024
	...
	"raftstore.follower-read-max-log-gap": int | *100
	...
	"raftstore.slow-trend-unsensitive-cause": int | *10
	...
	"raftstore.slow-trend-unsensitive-result": float | *0.5
	...
	"coprocessor.batch-split-limit": int & >=1 | *10
	...
	"raftdb.wal-ttl-seconds": int & >=0 | *0

	// The size limit of the archived WAL files. When the value is exceeded, the system deletes these files. Unit: B|KiB|MiB|GiB. Default value: 0
	"raftdb.wal-size-limit": string
	...
	"raftdb.info-log-keep-log-file-num": int & >=0 | *10
	...
	"raft-engine.recovery-threads": int & >=1 | *4
	...
	"raft-engine.compression-level": int | *1
	...
	// The number of threads used in log backup. Value range: [2, 12]. Default value: CPU * 0.5
	"log-backup.num-threads": int
	...
	"cdc.incremental-scan-threads": int | *4
	...
	"cdc.incremental-scan-concurrency": int | *6
	...
	"resolved-ts.scan-lock-pool-size": int | *2
	...
	"causal-ts.renew-batch-min-size": int | *100
	...
	"causal-ts.renew-batch-max-size": int | *8192
