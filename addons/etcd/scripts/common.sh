...
get_current_leader() {
  ...
  peer_endpoints=$(exec_etcdctl_no_check_tls "$leader_endpoint" member list | awk -F', ' '{print $5}' | tr '\n' ',' | sed 's#,$##')
  leader_endpoint=$(exec_etcdctl_no_check_tls "$peer_endpoints" endpoint status | awk -F', ' '$5=="true" {print $1}')
