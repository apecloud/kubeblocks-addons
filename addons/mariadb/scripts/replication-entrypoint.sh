...
reconcile_sql_listener_for_syncer_secondary_once() {
  ...
  local now role slave_status slave_rejoin_rc
  [ ! -f "${DATA_DIR}/.prestop-fence-started" ] || return 0
  role="$(query_local_syncer_role || true)"
  [ "${role}" = "secondary" ] || return 0
  # Fresh pod-0 bootstrap briefly publishes a writable local primary before
  # syncer has created its DCS leader ConfigMap. On syncer builds where an
  # absent DCS lease reports the default role "secondary", immediately
  # applying the secondary fence closes that only election window: syncer
  # never observes IsLeader=true and both replicas wait forever. Preserve the
  # deterministic pod-0 bootstrap state for the same bounded grace period used
  # by the primary reconciler. Once DCS publishes a role, master.info appears,
  # or the grace expires, the normal fail-closed secondary path resumes.
  if [ "${POD_INDEX}" = "0" ] && [ ! -f "${DATA_DIR}/master.info" ] && local_primary_role_published; then
    now="$(date +%s)"
    if [ "${now}" -lt "${SYNCER_PRIMARY_BOOTSTRAP_GRACE_UNTIL}" ]; then
      prestop_watchdog_log "runtime-secondary-listener-reconcile-defer reason=fresh-pod0-dcs-bootstrap-grace pod_index=${POD_INDEX}"
      return 0
    fi
  fi
