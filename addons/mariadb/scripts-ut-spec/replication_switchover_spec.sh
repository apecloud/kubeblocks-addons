...
EOF
  ...
  Describe "alpha.75 v1: ensure_internal_local_admin probe table bootstrap"
    ...
    It "Chart.yaml literal version is current (alpha.21 - reconfigure argv contract)"
      chart_yaml="${SHELLSPEC_CWD:?}/addons/mariadb/Chart.yaml"
      When call grep -c '^version: 1.2.0-alpha.21$' "${chart_yaml}"
      The output should equal "1"
    End
...
EOF
  ...
  Describe "alpha.65 v1 chart version bump for CmpD immutability"
    ...
    It "alpha.65 v1: Chart.yaml chart bump pattern from alpha.64 due to CmpD immutability — current bumped further to alpha.21 [contract-no-regression]"
      ...
      The status should be success
      The output should equal "version: 1.2.0-alpha.21"
    End
  ...
  Describe "alpha.66 v1 syncer HA executor + chart bump"
    ...
    Context "chart bump for CmpD immutability (per alpha.65 lesson)"
      It "alpha.66 v1: Chart.yaml chart bump pattern locked — current bumped to alpha.21 [contract-no-regression]"
  ...
  Describe "alpha.67 v1 ensure_internal_local_admin write-site zero-priv enforcement"
    ...
    Context "chart bump alpha.66 → alpha.67 → alpha.68 (CmpD immutability rule)"
      It "alpha.67 v1: Chart.yaml chart bump pattern locked — current bumped to alpha.21 [contract-no-regression]"
