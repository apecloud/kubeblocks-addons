...
Describe "cmpd-replication.yaml semisync startup recovery"
  ...
  fresh_pod0_primary_is_not_demoted_before_dcs_bootstrap() {
    awk '
      index($0, "reconcile_sql_listener_for_syncer_secondary_once() {") { fn = 1 }
      fn && index($0, "[ \"${POD_INDEX}\" = \"0\" ]") { pod0 = NR }
      fn && index($0, "local_primary_role_published") { published = NR }
      fn && index($0, "SYNCER_PRIMARY_BOOTSTRAP_GRACE_UNTIL") { grace = NR }
      fn && index($0, "fresh-pod0-dcs-bootstrap-grace") { defer = NR }
      fn && index($0, "set_replica_read_only \"runtime-secondary-reconcile\"") { fence = NR; exit }
      END { exit(pod0 && published && grace && defer && fence && pod0 < fence && published < fence && grace < fence && defer < fence ? 0 : 1) }
    ' "$(template_file)"
  }
  ...
  It "preserves fresh pod-0 primary long enough for syncer to initialize DCS"
    When call fresh_pod0_primary_is_not_demoted_before_dcs_bootstrap
    The status should be success
  End
