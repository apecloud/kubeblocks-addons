...
export CLICKHOUSE_ADMIN_USER=${KB_SERVICE_USER}
export CLICKHOUSE_ADMIN_PASSWORD=${KB_SERVICE_PASSWORD}
...
function create_databases() {
   ...
   execute_sql "$pod_fqdn" "select name from system.databases WHERE name NOT IN ('system', 'INFORMATION_SCHEMA','information_schema')" | while read -r db; do
       ...
       execute_sql "${KB_NEW_MEMBER_POD_IP}" "$create_query;"
   done
}
...
function create_tables() {
   ...
   execute_sql "${pod_fqdn}" "SELECT database,name, uuid FROM system.tables WHERE database NOT IN ('system', 'INFORMATION_SCHEMA','information_schema') order by dependencies_table desc" | while read -a row; do
     ...
     execute_sql "${KB_NEW_MEMBER_POD_IP}" "$query;"
     if [[ $? -eq 253 ]]; then
       echo "Replicas already exists, will drop the replica"
       execute_sql "${pod_fqdn}" "SYSTEM DROP REPLICA '${KB_NEW_MEMBER_POD_NAME}' FROM TABLE ${database}.${table}"
       execute_sql "${KB_NEW_MEMBER_POD_IP}" "$query;"
     fi
   done
}
...
for pod_fqdn in $(echo "$KB_MEMBER_ADDRESSES" | tr ',' '\n'); do
  pod_fqdn=${pod_fqdn%:*}
  if [ "$pod_fqdn" == "${KB_NEW_MEMBER_POD_NAME}*" ]; then
