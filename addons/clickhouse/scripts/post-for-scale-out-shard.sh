...
function get_new_shard_and_old_available_pod_fqdns() {
	...
	for component in "${!pods_map[@]}"; do
		...
		for pod_fqdn in $(echo $shard_pod_fqdn_list | tr ',' ' '); do
			tableCount=$(ch_query "$pod_fqdn" "SELECT count(name) FROM system.tables WHERE database NOT IN ('system', 'INFORMATION_SCHEMA','information_schema')")
...
function create_databases() {
	...
	ch_query "$pod_fqdn" "select name from system.databases WHERE name NOT IN ('system', 'INFORMATION_SCHEMA','information_schema')" | while read -r db; do
		...
		ch_query "${new_pod_fqdn}" "$create_query;"
		databaseCount=$(ch_query "${new_pod_fqdn}" "SELECT count(name) FROM system.databases WHERE name='${db}'")
...
function create_tables() {
	...
	ch_query "$pod_fqdn" "SELECT database,name, uuid FROM system.tables WHERE database NOT IN ('system', 'INFORMATION_SCHEMA','information_schema') order by dependencies_table desc" | while read -a row; do
		...
		tableCount=$(ch_query "${new_pod_fqdn}" "SELECT count(name) FROM system.tables WHERE database='${database}' and name='${table}'")
...
function get_available_pod_fqdn() {
	for pod_fqdn in "${old_shard_available_pod_fqdns[@]}"; do
		ch_query "$pod_fqdn" "select 1" >/dev/null 2>&1
		if [[ $? -eq 0 ]]; then
			echo "$pod_fqdn"
			break
		fi
