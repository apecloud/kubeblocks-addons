...
Describe "RustFS stable ComponentDefinition contract"
  ...
  validate_stable_definition_contract() {
    ...
    ruby -ryaml -e '
      ...
      addon_document_sets = {
        "install" => hash_documents.call(File.read(ARGV.fetch(2))),
        "upgrade" => hash_documents.call(File.read(ARGV.fetch(3)))
      }
      cluster_documents = hash_documents.call(File.read(ARGV.fetch(4)))
      ...
      addon_document_sets.each do |lifecycle, addon_documents|
        cmpd = addon_documents.find { |document| document["kind"] == "ComponentDefinition" }
        abort "#{lifecycle}: missing RustFS ComponentDefinition" unless cmpd
        abort "#{lifecycle}: ComponentDefinition identity must remain #{expected_cmpd}" unless cmpd.dig("metadata", "name") == expected_cmpd
        abort "#{lifecycle}: ComponentDefinition must use beta.10" unless cmpd.dig("spec", "serviceVersion") == expected_service_version
        annotations = cmpd.dig("metadata", "annotations") || {}
        abort "#{lifecycle}: ordinary uninstall must not retain the ComponentDefinition" if annotations["helm.sh/resource-policy"] == "keep"
        abort "#{lifecycle}: stable-name render must not bypass immutable checks" if annotations.key?("apps.kubeblocks.io/skip-immutable-check")

        cmpv = addon_documents.find { |document| document["kind"] == "ComponentVersion" }
        comp_defs = cmpv&.dig("spec", "compatibilityRules", 0, "compDefs")
        abort "#{lifecycle}: ComponentVersion must select only #{expected_cmpd}" unless comp_defs == ["^rustfs-0\\.1\\.0$"]

        hooks = addon_documents.select { |document| document.dig("metadata", "annotations", "helm.sh/hook") }
        abort "ordinary #{lifecycle} must not add migration hooks" unless hooks.empty?
      end
    ...
    ' "${tmp_dir}/addons/rustfs/Chart.yaml" \
      ...
      "$addon_rendered" "$addon_upgrade_rendered" "$cluster_rendered"
  }
