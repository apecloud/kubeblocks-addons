#RedisParameter: {
	...
	pidfile: string | *"/var/run/redis_6379.pid"

	logfile: string | *"/data/running.log"

	dbfilename: string | *"dump.rdb"

	dir: string | *"/data"

	"acllog-max-len": int & >=0 | *128
	...
	aclfile: string | *"/etc/redis/users.acl"

	"io-threads": int & >=1 | *4

	"io-threads-do-reads": string & "yes" | "no" | *"yes"

	"hash-max-ziplist-entries": int & >=0 | *512

	"hash-max-ziplist-value": int & >=0 | *64

	"list-max-ziplist-size": int | *-2

	"zset-max-ziplist-entries": int & >=0 | *128

	"zset-max-ziplist-value": int & >=0 | *64

	"cluster-config-file": string | *"/data/nodes.conf"

	"cluster-allow-replica-migration": string & "yes" | "no" | *"no"

	"cluster-allow-reads-when-down": string & "yes" | "no" | *"no"
	...
}
