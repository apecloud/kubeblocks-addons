  ...
  for index in "${!pod_ips[@]}"; do
    ...
    if [ -n "$REDIS_CLUSTER_ALL_SHARDS_HOST_NETWORK_PORT" ] && [ -n "$HOST_NETWORK_ENABLED" ]; then
      ...
      for mapping in "${port_mappings[@]}"; do
        ...
        if echo "${pod_names[$index]}" | grep -q "$shard_name"; then
          port=$mapping_port
          break
        fi
      done
    ...
    # redis v6.x does not support hostname/fqdn, use pod ip instead
    other_undeleted_component_nodes+=("${pod_ips[$index]}:$port")
  done
...
is_redis_cluster_initialized() {
  if [ -z "$KB_CLUSTER_POD_IP_LIST" ] || [ -z "$KB_CLUSTER_POD_NAME_LIST" ]; then
    echo "Error: Required environment variable KB_CLUSTER_POD_IP_LIST and KB_CLUSTER_POD_NAME_LIST is not set."
    exit 1
  fi
  ...
  for pod_name in $(echo "$KB_CLUSTER_POD_NAME_LIST" | tr ',' ' '); do
    local pod_ip=$(parse_host_ip_from_built_in_envs "$pod_name" "$KB_CLUSTER_POD_NAME_LIST" "$KB_CLUSTER_POD_IP_LIST")
    if [ -z "$pod_ip" ]; then
      echo "Failed to get the host ip of the pod $pod_name in is_redis_cluster_initialized"
      continue
    fi
    ...
    while [ $retry_count -lt $max_retries ]; do
      set +x
      cluster_info=$(redis-cli -h "$pod_ip" -p "$port" -a "$REDIS_DEFAULT_PASSWORD" cluster info)
      status=$?
      set -x
      ...
      if [ $status -eq 0 ]; then
        ...
        if [ $retry_count -lt $max_retries ]; then
          echo "Attempt $retry_count/$max_retries: Failed to get cluster info from $pod_ip port:$port, retrying in $retry_interval seconds..."
          sleep $retry_interval
        else
          echo "All $max_retries attempts failed to get cluster info from $pod_ip port:$port"
        fi
      fi
    done
...
get_current_comp_nodes_for_scale_in() {
  ...
  # 4958e6dca033cd1b321922508553fab869a29d 172.10.0.1:1050@1051 - 0 1711958289570 4 connected 0-1364 5461-6826 10923-12287
  while read -r line; do
    # 10.42.0.227:6379@16379
...
init_current_comp_default_nodes_for_scale_out() {
  ...
  for pod_name in $(echo "$KB_CLUSTER_COMPONENT_POD_NAME_LIST" | tr ',' ' '); do
    ...
    ## if the REDIS_CLUSTER_ADVERTISED_PORT is set, use the advertised port (currently only support NodePort, LoadBalancer will be supported in the future)
    ...
    ## deal with host network mode
      ...
      if [ "$pod_name_ordinal" -eq 0 ]; then
        scale_out_shard_default_primary_node["$pod_name"]="$pod_host_ip:$REDIS_CLUSTER_HOST_NETWORK_PORT"
      else
        scale_out_shard_default_other_nodes["$pod_name"]="$pod_host_ip:$REDIS_CLUSTER_HOST_NETWORK_PORT"
      fi
    else
      local port=$SERVICE_PORT
      mapping_pod_ip=$(parse_host_ip_from_built_in_envs "$pod_name" "$KB_CLUSTER_COMPONENT_POD_NAME_LIST" "$KB_CLUSTER_COMPONENT_POD_IP_LIST")
      if [ -z "$mapping_pod_ip" ]; then
        echo "Failed to get the mapping pod ip of the pod $pod_name in host network mode"
        exit 1
      fi
      if [ "$pod_name_ordinal" -eq 0 ]; then
        scale_out_shard_default_primary_node["$pod_name"]="$mapping_pod_ip:$port"
      else
        scale_out_shard_default_other_nodes["$pod_name"]="$mapping_pod_ip:$port"
      fi
    fi
  done
}
...
gen_initialize_redis_cluster_node() {
  ...
  for pod_name in $(echo "$KB_CLUSTER_POD_NAME_LIST" | tr ',' ' '); do
    ...
    ## the value format of REDIS_CLUSTER_ALL_SHARDS_HOST_NETWORK_PORT is "shard-chg:1060,shard-khh:1059,shard-mpg:1054"
      ...
      IFS=','
      set -f
      read -ra shard_ports <<< "$REDIS_CLUSTER_ALL_SHARDS_HOST_NETWORK_PORT"
      set +f
      IFS="$old_ifs"

      for shard_port in "${shard_ports[@]}"; do
        shard_name=$(echo "$shard_port" | cut -d':' -f1)
        port=$(echo "$shard_port" | cut -d':' -f2)
        ...
        if [ "$is_primary" = true ]; then
          initialize_redis_cluster_primary_nodes["$pod_name"]="$pod_host_ip:$port"
        else
          initialize_redis_cluster_secondary_nodes["$pod_name"]="$pod_host_ip:$port"
        fi
        initialize_pod_name_to_advertise_host_port_map["$pod_name"]="$pod_host_ip:$port"
        break
      done
    else
      # redis v6.x does not support hostname/fqdn addr, so use pod ip by default
      local port=$SERVICE_PORT
      mapping_pod_ip=$(parse_host_ip_from_built_in_envs "$pod_name" "$KB_CLUSTER_POD_NAME_LIST" "$KB_CLUSTER_POD_IP_LIST")
      if [ -z "$mapping_pod_ip" ]; then
        echo "Failed to get the mapping pod ip of the pod $pod_name"
        exit 1
      fi
      if [ "$is_primary" = true ]; then
        initialize_redis_cluster_primary_nodes["$pod_name"]="$mapping_pod_ip:$port"
      else
        initialize_redis_cluster_secondary_nodes["$pod_name"]="$mapping_pod_ip:$port"
      fi
      initialize_pod_name_to_advertise_host_port_map["$pod_name"]="$mapping_pod_ip:$port"
    fi
  done
}
...
initialize_redis_cluster() {
  ...
  for secondary_pod_name in "${!initialize_redis_cluster_secondary_nodes[@]}"; do
    ...
    if ! verify_secondary_in_all_primaries "$secondary_pod_ip" "${primary_node_list[@]}"; then
      echo "Failed to verify secondary node $secondary_pod_ip in all primary nodes"
      exit 1
    fi
...
verify_secondary_in_all_primaries() {
  ...
  for primary_node in "$@"; do
    ...
    retry_count=0
    while ! is_node_in_cluster "$primary_host" "$primary_port" "$secondary_node_ip" && [ $retry_count -lt 30 ]; do
      sleep 3
      ((retry_count++))
    done
    ...
    if [ $retry_count -eq 30 ]; then
      echo "Secondary node $secondary_node_ip not found in primary $primary_node after 30 seconds"
      return 1
    fi
  done
  return 0
