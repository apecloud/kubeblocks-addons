...
get_current_comp_nodes_for_scale_out_replica() {
  ...
  parse_node_line_info() {
    ...
    printf "%s %s %s %s" "$node_announce_ip" "$node_port" "$node_bus_port" "$node_role"
  }
  ...
  build_node_entry() {
    ...
    local port="$3"
    local bus_port="$4"
    echo "$announce_ip#$announce_ip:$port@$bus_port"
  }
...
scale_redis_cluster_replica() {
  ...
  forget_fail_node_when_cluster_is_ok "$primary_node_endpoint" "$primary_node_port"
...
build_announce_ip_and_port() {
  ...
  if ! is_empty "$redis_announce_host_value" && ! is_empty "$redis_announce_port_value"; then
    ...
    echo "redis use pod id $CURRENT_POD_IP to announce"
    echo "replica-announce-ip $CURRENT_POD_IP" >> $redis_real_conf
  fi
}
