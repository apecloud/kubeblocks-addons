...
scale_redis_cluster_replica() {
  ...
  if ! is_rebuild_instance && is_node_in_cluster "$primary_node_endpoint_for_meet" "$primary_node_port" "$current_pod_name"; then
  ...
  primary_node_cluster_id=$(get_cluster_id "$primary_node_endpoint_for_meet" "$primary_node_port")
  ...
  if is_rebuild_instance; then
    ...
    node_id=$(get_cluster_id "$primary_node_endpoint_for_meet" "$primary_node_port" "$current_pod_fqdn")
    if [ -z ${REDIS_DEFAULT_PASSWORD} ]; then
