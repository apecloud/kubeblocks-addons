...
retry_get_master_addr_by_name_from_sentinel() {
  ...
  while [ $retry_count -lt $max_retry ]; do
    set +x
    echo "execute command: timeout $timeout_value redis-cli -h $sentinel_pod_addr -p $SENTINEL_SERVICE_PORT -a ******** sentinel get-master-addr-by-name $KB_CLUSTER_COMP_NAME"
    output=$(timeout "$timeout_value" redis-cli -h "$sentinel_pod_addr" -p "$SENTINEL_SERVICE_PORT" -a "$SENTINEL_PASSWORD" sentinel get-master-addr-by-name "$KB_CLUSTER_COMP_NAME")
    set -x
    exit_code=$?
    ...
    if [ $exit_code -eq 0 ]; then
      ...
      if [ "${#REDIS_SENTINEL_PRIMARY_INFO[@]}" -eq 2 ] && [ -n "${REDIS_SENTINEL_PRIMARY_INFO[0]}" ] && [ -n "${REDIS_SENTINEL_PRIMARY_INFO[1]}" ]; then
        echo "Successfully retrieved primary info from sentinel: $sentinel_pod_addr"
        return 0
      else
        echo "Empty primary info retrieved from sentinel: $sentinel_pod_addr"
        return 0
      fi
    else
      if [ $exit_code -eq 124 ]; then
        echo "Timeout occurred while retrieving primary info from sentinel: $sentinel_pod_addr. Retrying..."
      else
        echo "Error occurred while retrieving primary info from sentinel: $sentinel_pod_addr. Retrying..."
      fi
    fi
  ...
  echo "Failed to retrieve primary info from sentinel: $sentinel_pod_addr after $max_retry retries."
  return 1
}
