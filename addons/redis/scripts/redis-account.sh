...
function env_pre_check() {
    ...
    if [ -z "$KB_CLUSTER_NAME" ]; then
        echo "KB_CLUSTER_NAME is empty, skip ACL operation"
        exit 1
    fi

    if [ -z "$KB_NAMESPACE" ]; then
        echo "KB_NAMESPACE is empty, skip ACL operation"
        exit 1
    fi

    if [ -z "$KB_COMP_NAME" ]; then
        echo "KB_COMP_NAME is empty, skip ACL operation"
        exit 1
    fi
...
function get_cluster_host_list() {
    host_list=$(redis-cli -c -h "$KB_POD_NAME.${KB_CLUSTER_NAME}-${KB_COMP_NAME}-headless.${KB_NAMESPACE}.svc.cluster.local" \
...
function kb_pod_list_to_host_list() {
    IFS=',' read -ra PODS <<<"$KB_POD_LIST"
    res=()
    for pod in "${PODS[@]}"; do
        host="${pod}.${KB_CLUSTER_NAME}-${KB_COMP_NAME}-headless.${KB_NAMESPACE}.svc.cluster.local"
        res+=("$host")
    done
    host_list=$(
        IFS=,
