...
get_role_from_redis_pod() {
  ...
  unset_xtrace_when_ut_mode_false
  if is_empty "$REDIS_DEFAULT_PASSWORD"; then
    output=$(timeout 1 redis-cli $REDIS_CLI_TLS_CMD -h "$redis_pod_fqdn" -p "$service_port" ROLE 2>&1)
  else
    output=$(REDISCLI_AUTH="$REDIS_DEFAULT_PASSWORD" timeout 1 redis-cli $REDIS_CLI_TLS_CMD -h "$redis_pod_fqdn" -p "$service_port" ROLE 2>&1)
  fi
  status=$?
  set_xtrace_when_ut_mode_false
