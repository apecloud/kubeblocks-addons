...
build_redis_default_accounts() {
  ...
  if ! is_empty "$REDIS_SENTINEL_PASSWORD"; then
    ...
    if [[ "$SERVICE_VERSION" == 6.0* ]]; then
      # Redis 6.0 does not support the allchannels keyword (added in Redis 6.2)
      echo "user $REDIS_SENTINEL_USER on +multi +slaveof +ping +exec +subscribe +config|rewrite +role +publish +info +client|setname +client|kill +script|kill #$redis_sentinel_password_sha256" >> $redis_acl_file
    else
