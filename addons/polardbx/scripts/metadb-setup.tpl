...
function generate_dn_init_sql() {
    ...
    do
      ...
      VALUES (NULL, NOW(), NOW(), '$CLUSTER_NAME', '$DN_NAME', '$DN_NAME', '$DN_HOSTNAME', '3306', '31600', '$metaDbUser', '$ENC_PASSWORD', '3', '0', '0', NULL, NULL, NULL, 10000, 4,  34359738368 , '0', '');"
      echo $dn_init_sql >> /scripts/gms-init-metadata.sql
    done
    echo "UPDATE config_listener SET op_version = op_version + 1 WHERE data_id = 'polardbx.storage.info.$CLUSTER_NAME'" >> /scripts/gms-init-metadata.sql
}
...
GMS_HOST=$GMS_SVC_NAME"."$CLUSTER_NAMESPACE".svc.cluster.local"
