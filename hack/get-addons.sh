...
for d in $(find ./addons -type d  -not -name "common" -not -name "kblib" -not -name "neonvm" | sort); do
  ...
  if [[ -f "$d/Chart.yaml" ]]; then
    ...
    # if version_lines is empty, try to get version from ClusterVersioin
    if [[ -z $version_lines ]]; then
      version_lines=$(cat "/tmp/rendered.yaml"  | yq e '. | select(.kind == "ClusterVersion") | .spec.componentVersions[].versionsContext.containers[].image |sub(".*/", "") | sub(":", "-")' -N| sort | uniq)
    fi
