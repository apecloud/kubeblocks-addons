...
#    - cryptography library (pip install cryptography) - Required for MySQL 8.0+ auth
...
#    - Handles MySQL 8.0+ authentication methods (caching_sha2_password)
#
# 📞 SUPPORT:
...
- PyMySQL and cryptography packages
...
# Check for cryptography package (required for MySQL 8.0+ authentication)
try:
    import cryptography
except ImportError:
    print("❌ Error: cryptography package is required for MySQL 8.0+ authentication")
...
class MySQLSysbench:
    ...
    def _get_connection(self, database=None):
        """Get a database connection using PyMySQL with proper authentication support"""
        ...
        try:
            ...
            if 'cryptography' in error_msg:
                raise Exception(
                    ...
                    "Solution: Install cryptography package with: pip install cryptography"
                )
            elif 'caching_sha2_password' in error_msg or 'sha256_password' in error_msg:
                raise Exception(
                    f"Authentication method not supported: {error_msg}\n"
                    "Solution: Either:\n"
                    "1. Install cryptography package: pip install cryptography\n"
                    "2. Change MySQL user auth plugin: ALTER USER 'user'@'host' IDENTIFIED WITH mysql_native_password BY 'password';"
                )
            else:
